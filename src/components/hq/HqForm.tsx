...
export function HqForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(typeof data.error === "string" ? data.error : "Failed to create HQ")
      return
    }

    const hq = await res.json()
    router.push("/hq/" + hq.slug)
  }

  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto space-y-5 py-10 px-4">
      <h1 className="text-2xl font-semibold text-white">Create an HQ</h1>
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            { value: "PUBLIC", label: "Public", hint: "Anyone can find and view this HQ" },
            { value: "PRIVATE", label: "Private", hint: "Members only, hidden from browse" },
          ].map((opt) => (
            <label
              key={opt.value}
